...
fn evaluate_equation(equation: &Equation) -> bool {
    ...
    let mut concat_terms: VecDeque<i64> = equation.terms.clone();
    let first_term = concat_terms.pop_front().unwrap().to_string();
    let second_term = concat_terms.pop_front().unwrap().to_string();
    concat_terms.push_front((first_term + &second_term).parse::<i64>().unwrap());
